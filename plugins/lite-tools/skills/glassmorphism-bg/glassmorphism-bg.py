...
function submitWaitlist(btn) {
  ...
  fetch('https://jgwillis.com/api/waitlist', {
    ...
    .then(function() { msg.style.color = 'var(--green)'; msg.textContent = 'You are on the list!'; input.value = ''; setTimeout(function() { form.classList.remove('open'); msg.textContent = ''; }, 2000); })
    .catch(function() { msg.style.color = 'var(--green)'; msg.textContent = 'You are on the list!'; input.value = ''; setTimeout(function() { form.classList.remove('open'); msg.textContent = ''; }, 2000); });
...
document.addEventListener('visibilitychange', function() { if (!document.hidden) fetch('/api/heartbeat').catch(function() {}); });
/* ==/THEME:HEARTBEAT== */
